...
1.  Download the latest installer script from the [download page](https://github.com/conda-forge/miniforge/releases/latest),
    ...
    <Tabs syncKey="miniforge-install">
...
3.  Choose **no** when prompted whether you want to update your shell configuration or not.
...
5.  Close and reopen WSL (so the `.bashrc` gets sourced), or temporarily add miniforge to you path

    ```nu
    $env.PATH = ($env.PATH | append $"($env.HOME)/miniforge3/bin")
    ```

6.  Make sure the conda plugin for `nu` works correctly
...
Now when you want to activate an environment, you can simply use `conda activate`.
